...
function logout() {
    isLoggedIn = false;
    updateHeaderButtons();
    // Xóa thông tin đăng nhập khỏi local storage hoặc session nếu bạn có lưu trữ
    console.log("Đăng xuất thành công");
  
    // Chuyển hướng về trang chủ hoặc trang khác tùy theo yêu cầu của bạn
    window.location.href = '/Homepage/Homepage.html'; 
  }
  ...
  function addToCart() {
    if (!isLoggedIn) {
      alert("Vui lòng đăng nhập để thêm sản phẩm vào giỏ hàng."); // Hoặc bạn có thể mở modal đăng nhập ở đây
      return;
    }
  
    // Logic để thêm sản phẩm vào giỏ hàng (Bạn sẽ cần triển khai phần này dựa trên cách bạn lưu trữ giỏ hàng)
